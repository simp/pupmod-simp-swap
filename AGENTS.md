...
- **Static mode (default).** It declares a single `sysctl { 'vm.swappiness' }`
  ...
  is `absent` (`init.pp`). This is the recommended mode for most systems
...
sense unless it is run reasonably often" (`init.pp`).
...
- **`swap` (`manifests/init.pp`)** — Public entry class (not
  ...
  - **Dynamic branch** (`init.pp`): declares
  ...
  - **Static branch** (`init.pp`): declares
...
- **The two modes are mutually exclusive, and switching between them cleans
  ...
  absent }` (`init.pp`), so flipping `$dynamic_script` false removes the
...
- **The cron job runs the script with no flags.** The CLI override flags
  ...
  always uses the ERB-baked defaults (`init.pp`).
...
- **`$swappiness` is applied verbatim.** The docstring warns the value is used
  "as-is" (`init.pp`); the only guard is the `Integer[0,100]` type.
...
- `puppet/augeasproviders_sysctl` `>= 2.4.0 < 7.0.0` (provides the `sysctl`
  type/provider used by the static branch) (`metadata.json`).
- `simp/simplib` `>= 4.9.0 < 6.0.0` (`metadata.json`).
...
`metadata.json`): **`openvox >= 8.0.0 < 9.0.0`.** This module has been
...
(`['openvox', 'puppet'].each do |gem_name| ...`, `Gemfile`; default range
`['>= 8', '< 9']`, `Gemfile`); this shim goes away once the `puppet`
...
- **Acceptance runs in CI:** `.github/workflows/pr_tests.yml` defines six
  ...
  (`pr_tests.yml`). The acceptance job runs a matrix of `almalinux9`
  and `almalinux10` (`pr_tests.yml`), provisions libvirt/Vagrant, and
  runs `bundle exec rake beaker:suites[default,<node>]` under
  `BEAKER_HYPERVISOR=vagrant_libvirt` (`pr_tests.yml`). Unit specs run
  against Puppet 8.x on Ruby 3.2 (`pr_tests.yml`).
...
`~> 1.88.0` (`Gemfile`). The default OpenVox/Puppet test range is
`['>= 8', '< 9']` (`Gemfile`), and both gems are installed during the
migration shim (`Gemfile`).
...
- Keep the two modes mutually exclusive and self-cleaning: the static branch
  ...
  doesn't leave a stray job behind (`init.pp`).
